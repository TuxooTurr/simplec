...
# 15 МБ. Замер на синтетическом логе 18 МБ / 174k строк: полный проход с
# поиском маркеров ошибок занимает 0.21 с — размер файла узким местом не является.
# Поэтому НЕ режем вход по числу строк или по «последним N минутам»: и то и другое
# выбросило бы ошибки из середины файла, ничего не выиграв по скорости (обрезка до
# 1000 строк на том же файле нашла 508 строк-ошибок вместо 89 537).
_UPLOAD_MAX_BYTES = 15 * 1024 * 1024
# Реальное ограничение — сколько текста уходит в LLM. Его и подняли.
_EXCERPT_MAX_CHARS = 40_000
_ERROR_MARKERS = (
