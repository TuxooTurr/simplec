...
class LayeredGenerator:
    ...
    def generate_case_markdown(self, case_info, qa_doc):
        ...
        prompt = (
            ...
            "- Вся техника живёт в зонах API и БД — там пиши ПОДРОБНО:\n"
            "  API — метод, эндпоинт, заголовки, тело запроса, код ответа и структура ответа;\n"
            "  БД — схема и таблица, конкретные поля и их значения после шага.\n"
    ...
    def wrap_case_to_xml_via_llm(self, case, qa_doc,
        ...
        prompt = (
            ...
            "«Не требуется», саму зону не пропускай. Зоны API и БД пиши ПОДРОБНО: в API — метод, эндпоинт, "
            "заголовки, тело запроса, код и структуру ответа; в БД — схему, таблицу, поля и их значения\n"
            "3.1. description — строго бизнес-язык: что делает человек в терминах предметной области. "
            "В шаге НЕ должно быть HTTP-методов, эндпоинтов, curl, JSON, SQL, имён таблиц и кодов ответов — "
            "всё это переносится в зоны API и БД. Бери формулировку шага из Markdown-кейса; если она "
            "техническая — переформулируй её на бизнес-язык, а технику убери в нужную зону\n"
