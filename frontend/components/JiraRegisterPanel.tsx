...
import {
  ...
  createJiraDefect, type JiraProjectMeta, type JiraEpic, type JiraUser, type JiraSettings,
} from "@/lib/jiraApi";
  ...
  return (
    <div className="bg-bg-card border border-border-main rounded-xl p-5 mb-4 animate-slide-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
        <div>
          <label className={LBL}>Проект *</label>
          <div className="flex gap-2">
            <input value={project} onChange={e => setProject(e.target.value.toUpperCase())}
              onKeyDown={e => e.key === "Enter" && loadMeta(project)}
              placeholder="SBER911" className={`${INPUT_CLS} font-mono uppercase`} />
            <button onClick={() => loadMeta(project)} disabled={!project.trim() || metaLoading}
              className="px-3 py-2 border border-border-main rounded-lg text-xs font-semibold text-text-muted hover:bg-bg-subtle disabled:opacity-40 whitespace-nowrap">
              {metaLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "Загрузить"}
            </button>
          </div>
          {metaErr && <p className="text-xs text-red-500 mt-1">{metaErr}</p>}
          {meta && <p className="text-xs text-green-600 mt-1">Тип задачи: {meta.issuetype}</p>}
        </div>
