...
import {
  ...
  type AlertScript, type DynamicParam, type NotebookCell,
} from "@/lib/api";
...
function newParam(): DynamicParam {
  return { id: `param-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`, label: "", code_key: "", placeholder: "" };
}
...
function NotebookEditor({ cells, onChange }: { cells: NotebookCell[]; onChange: (c: NotebookCell[]) => void }) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      // Imported cells come as "code" → treat as "init"
  ...
  return (
    <div className="border border-border-main rounded-xl overflow-hidden">
      <div className="flex items-center border-b border-border-main bg-gray-50">
        {(["editor", "upload"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`flex items-center gap-1.5 px-4 py-2 text-xs font-medium transition-colors ${
              tab === t ? "text-primary border-b-2 border-primary bg-white" : "text-text-muted hover:text-text-main"}`}>
          ...
          {cells.map((cell, idx) => {
            ...
            return (
              <div key={cell.id} className={`border rounded-lg overflow-hidden ${cfg.border}`}>
                <div className={`flex items-center gap-1.5 px-2 py-1 border-b ${cfg.border} bg-white/60`}>
                ...
                <textarea value={cell.source} onChange={e => updateSrc(cell.id, e.target.value)}
                  ...
                  className="w-full px-3 py-2 text-xs font-mono resize-y focus:outline-none focus:bg-white/80 transition-colors" />
              </div>
            );
          })}
        ...
        <div className="p-4">
          ...
          <button onClick={() => fileRef.current?.click()} disabled={uploading}
            className="w-full flex flex-col items-center gap-2 py-8 border-2 border-dashed border-border-main rounded-xl
              text-text-muted hover:border-primary/50 hover:text-primary hover:bg-indigo-50/30 disabled:opacity-50 transition-all">
            {uploading
...
function ScriptModal({ initial, onSave, onClose }: {
  ...
  const [params, setParams] = useState<DynamicParam[]>(initial?.dynamic_params ?? []);
  const [saving, setSaving] = useState(false);
  const [err,    setErr]    = useState("");

  const addParam    = () => setParams(ps => [...ps, newParam()]);
  const removeParam = (id: string) => setParams(ps => ps.filter(p => p.id !== id));
  const updateParam = (id: string, field: keyof DynamicParam, val: string) =>
  ...
  const handleSave = async () => {
    ...
    try {
      const saved = await saveAlertScript({ id: initial?.id, name: name.trim(), topic: "", notebook: cells, dynamic_params: params });
        ...
        <div className="overflow-y-auto scrollbar-thin p-6 space-y-5 flex-1">
          ...
          <div className="flex gap-3 text-xs">
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-50 text-green-700 border border-green-200">
              <Code className="w-3 h-3" /> <b>Init</b> — один раз при запуске (или при смене параметра)
            </span>
          ...
          <div>
            ...
            {params.length === 0 ? (
              <p className="text-xs text-text-muted bg-gray-50 rounded-lg px-3 py-3">
              ...
              <div className="space-y-2">
                <div className="grid grid-cols-[1fr_1fr_1fr_28px] gap-2 px-1">
                  <span className="text-[10px] font-semibold text-text-muted uppercase tracking-wide">Имя параметра</span>
                ...
                {params.map(p => (
                  <div key={p.id} className="grid grid-cols-[1fr_1fr_1fr_28px] gap-2 items-center bg-gray-50 rounded-lg px-3 py-2">
                    ...
                    <button onClick={() => removeParam(p.id)}
                      className="flex items-center justify-center w-7 h-7 rounded-lg text-text-muted hover:text-red-500 hover:bg-red-50">
...
function OutputConsole({ lines }: { lines: OutputLine[] }) {
  ...
  if (lines.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-text-muted gap-2">
        ...
        <p className="text-xs">Вывод ядра появится здесь</p>
      </div>
    );
  }
    ...
    <div className="font-mono text-xs space-y-0.5 p-2">
      {lines.map((line, i) => (
        <div key={i} className={`flex gap-2 ${
          line.kind === "error"  ? "text-red-600" :
          line.kind === "system" ? "text-blue-500 italic" : "text-text-main"
        }`}>
          <span className="text-text-muted/50 flex-shrink-0 select-none">{line.ts}</span>
...
export default function AlertsSection() {
  ...
  const initValues = (s: AlertScript) => {
    const v: Record<string, string> = {};
    for (const p of s.dynamic_params) v[p.id] = values[p.id] ?? p.placeholder;
    setValues(v);
  };

  const handleSelectScript = (s: AlertScript) => {
    if (schedActive) stopSchedule();
    setSelectedId(s.id);
    initValues(s);
  };
  ...
  const handleDelete = async (s: AlertScript) => {
    ...
    try {
      ...
      setScripts(next);
      if (selectedId === s.id) {
        if (next.length > 0) { setSelectedId(next[0].id); initValues(next[0]); }
    ...
    <div className="p-6 overflow-y-auto scrollbar-thin animate-slide-up h-full flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">Jupyter-ядро · Init (однократно) + Loop (цикл)</p>
        </div>
      ...
      {scripts.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 text-center gap-3 text-text-muted">
          ...
          <p className="text-sm">Нет алертов. Создайте первый.</p>
        </div>
      ) : (
        <div className="flex gap-4 flex-1 min-h-0">

          {/* Alert list */}
          <div className="w-52 flex-shrink-0 bg-white border border-border-main rounded-xl p-3 overflow-y-auto scrollbar-thin flex flex-col gap-1">
            {scripts.map(s => (
              <div key={s.id} onClick={() => handleSelectScript(s)}
                className={`group flex items-center gap-2 px-3 py-2.5 rounded-lg cursor-pointer transition-all ${
                  selectedId === s.id ? "bg-indigo-50 border border-primary/30" : "hover:bg-gray-50 border border-transparent"}`}>
                <Bell className={`w-3.5 h-3.5 flex-shrink-0 ${selectedId === s.id ? "text-primary" : "text-text-muted"}`} />
                <span className={`text-sm flex-1 min-w-0 truncate ${selectedId === s.id ? "font-semibold text-primary" : "text-text-main"}`}>
                  {s.name}
                </span>
                <div className="hidden group-hover:flex items-center gap-0.5 flex-shrink-0">
                  <button onClick={e => { e.stopPropagation(); setEditScript(s); setShowModal(true); }}
                    className="p-1 rounded text-text-muted hover:text-primary hover:bg-indigo-50" title="Редактировать">
                    <Pencil className="w-3 h-3" />
                  </button>
                  <button onClick={e => { e.stopPropagation(); handleDelete(s); }}
                    className="p-1 rounded text-text-muted hover:text-red-500 hover:bg-red-50" title="Удалить">
            ...
            <div className="flex-1 min-w-0 grid grid-cols-2 gap-4">
              ...
              <div className="bg-white border border-border-main rounded-xl p-5 overflow-y-auto scrollbar-thin flex flex-col gap-4">
                ...
                <div className={`flex items-center gap-3 rounded-xl px-4 py-3 border ${
                  kernelAlive ? "bg-green-50 border-green-200" : "bg-gray-50 border-border-main"}`}>
                  <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                    kernelConnecting ? "bg-amber-400 animate-pulse" :
                    kernelAlive      ? "bg-green-500 animate-pulse" : "bg-gray-300"}`} />
              ...
              <div className="bg-white border border-border-main rounded-xl p-5 flex flex-col gap-3 overflow-hidden">
                ...
                <div className="flex-1 min-h-0 border border-border-main rounded-lg bg-gray-950 overflow-y-auto scrollbar-thin">
                  <div className="text-xs font-mono space-y-0 p-2 min-h-full">
                    {output.length === 0 ? (
                      <p className="text-gray-500 italic p-2">— ожидание вывода —</p>
                    ) : (
                      output.map((line, i) => (
                        <div key={i} className={`flex gap-2 leading-5 ${
                          line.kind === "error"  ? "text-red-400" :
                          line.kind === "system" ? "text-blue-400" : "text-green-300"}`}>
                          <span className="text-gray-600 flex-shrink-0 select-none">{line.ts}</span>
                          <pre className="whitespace-pre-wrap break-all flex-1">{line.text}</pre>
                        </div>
                      ))
                    )}
                      ...
                      {(["once", "periodic"] as const).map(m => (
                        <button key={m} onClick={() => { setSchedMode(m); if (m === "once") stopSchedule(); }}
                          className={`px-3 py-1 rounded-lg text-xs font-medium border transition-all ${
                            schedMode === m ? "border-primary bg-indigo-50 text-primary" : "border-border-main text-text-muted hover:border-primary/40"}`}>
                    ...
                    <>
                        ...
                        {FREQS.map(f => (
                          <button key={f.secs} onClick={() => setSchedFreq(f.secs)}
                            className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${
                              schedFreq === f.secs ? "border-primary bg-indigo-50 text-primary" : "border-border-main text-text-muted hover:border-primary/40"}`}>
                            {f.label}
                          </button>
                        ))}
                      </div>
                      ...
                      {(schedActive || schedCount > 0) && (
                        <div className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium ${
                          schedActive ? "bg-green-50 text-green-700 border border-green-200" : "bg-gray-50 text-text-muted border border-border-main"}`}>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => { stopSchedule(); }} disabled={!schedActive}
                    className="flex items-center gap-1.5 text-sm px-3 py-1.5 border border-border-main rounded-lg text-text-muted hover:bg-gray-50 disabled:opacity-30 transition-all">
                    <RotateCcw className="w-3.5 h-3.5" /> Сброс
                  </button>

                  <div className="ml-auto">
                    {!kernelAlive ? (
                      <button disabled className="flex items-center gap-2 px-3 py-1.5 bg-gray-200 text-gray-400 rounded-lg text-sm font-semibold cursor-not-allowed">
...
// Small helper to scroll output to bottom
