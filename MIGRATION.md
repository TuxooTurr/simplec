...
- **JPype/JVM**: .jar-драйвер грузится «на лету» на каждое подключение (через
  URLClassLoader), поэтому добавленную/заменённую библиотеку перезапускать бэкенд
  НЕ нужно. Требуется только установленная Java 17+.
- **Драйвер по пути (jar_path)**: если библиотека указана путём на диске (рекомендуемый
  способ), путь машинно-зависимый — после переноса переуказать его в «Настройке
  драйверов → Библиотека». Загруженные в приложение .jar (`data/jdbc_drivers/`)
  переносятся вместе с папкой.
