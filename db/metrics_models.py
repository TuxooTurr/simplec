...
class TestSystem(Base):
    ...
    started_by     = Column(String(255), nullable=True)   # legacy display field; no user owner
...
class TestMetric(Base):
    ...
    last_metadata_sent_at   = Column(DateTime, nullable=True)
    last_thresholds_sent_at = Column(DateTime, nullable=True)
    created_at              = Column(DateTime, server_default=func.now(), nullable=False)
