...
export default function EtalonsPage() {
  ...
  const handleReqFile = async (file: File) => {
    ...
    try {
      ...
      setReqText(result.text);
    } catch (err) {
      alert("Ошибка при парсинге файла: " + String(err));
    } finally {
      setReqFileLoading(false);
    }
  };
  ...
  const handleTcFile = async (file: File) => {
    ...
    try {
      const result = await parseFile(file);
      setTcText(result.text);
    } catch (err) {
      alert("Ошибка при парсинге файла: " + String(err));
    } finally {
      setTcFileLoading(false);
    }
  };
        ...
        <div className="max-w-4xl animate-slide-up">
          ...
          <div className="flex gap-3 mb-4">
            ...
            <button
              onClick={() => load(true)}
              disabled={refreshing}
              className="flex items-center gap-1.5 px-4 py-2 border border-border-main rounded-lg text-sm text-text-muted
                hover:bg-gray-50 hover:text-text-main transition-all duration-150 disabled:opacity-50"
            >
            ...
            <div className="bg-white border border-border-main rounded-xl p-5 mb-4 animate-slide-up">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-text-muted hover:bg-gray-100 hover:text-text-main transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-1.5 h-7">
                    ...
                    <div className="flex rounded-md border border-border-main overflow-hidden text-[11px]">
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1 transition-colors duration-150
                          ${reqMode === "text" ? "bg-indigo-50 text-primary font-semibold" : "text-text-muted hover:bg-gray-50"}`}
                      >
                        <AlignLeft className="w-3 h-3" />
                        Текст
                      </button>
                      <button
                        onClick={() => setReqMode("file")}
                        className={`flex items-center gap-1 px-2 py-1 border-l border-border-main transition-colors duration-150
                          ${reqMode === "file" ? "bg-indigo-50 text-primary font-semibold" : "text-text-muted hover:bg-gray-50"}`}
                      >
                  ...
                  {reqMode === "text" ? (
                    ...
                    <div className="flex flex-col gap-2 flex-1">
                      <FileDropZone
                        onFile={handleReqFile}
                        loading={reqFileLoading}
                        className="flex-1 min-h-[168px]"
                      />
                  ...
                  <div className="flex items-center justify-between mb-1.5 h-7">
                    ...
                    <div className="flex rounded-md border border-border-main overflow-hidden text-[11px]">
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1 transition-colors duration-150
                          ${tcMode === "text" ? "bg-indigo-50 text-primary font-semibold" : "text-text-muted hover:bg-gray-50"}`}
                      >
                        <AlignLeft className="w-3 h-3" />
                        Текст
                      </button>
                      <button
                        onClick={() => setTcMode("file")}
                        className={`flex items-center gap-1 px-2 py-1 border-l border-border-main transition-colors duration-150
                          ${tcMode === "file" ? "bg-indigo-50 text-primary font-semibold" : "text-text-muted hover:bg-gray-50"}`}
                      >
                        <Paperclip className="w-3 h-3" />
                        Файл
                      </button>
