...
def _collect_historical_points(metric_hash: str, metric_id: int, db: Session) -> list[dict]:
    """Собирает все успешно отправленные точки за последние 24 часа."""
    from agents.metrics_message_builder import HEALTH_TO_STATUS

    cutoff = datetime.now(timezone.utc) - timedelta(hours=24)
    logs = (
        db.query(GenerationLog)
        .filter(
            GenerationLog.test_metric_id == metric_id,
            GenerationLog.status == "success",
            GenerationLog.value_sent.isnot(None),
            GenerationLog.sent_at >= cutoff,
        )
        .order_by(GenerationLog.sent_at.asc())
        .all()
    )
    points: list[dict] = []
    for log in logs:
        ts = int(log.sent_at.timestamp()) if log.sent_at else 0
        points.append({
            "metric_hash":    metric_hash,
            "metric_value":   float(log.value_sent),
            "baseline_value": float(log.baseline_sent) if log.baseline_sent is not None else None,
            "metric_ts":      ts,
            "metric_status":  HEALTH_TO_STATUS.get(log.health_sent) if log.health_sent is not None else None,
        })
    return points
...
async def send_now(metric_id: int, db: Session = Depends(get_db)):
    ...
    # Накопление: собираем все успешные точки за последние 24 часа
    historical_points = _collect_historical_points(m.metric_hash, metric_id, db)

    # Сообщения
    data_msg = build_data_message(m.metric_hash, value, baseline, health, historical_points=historical_points)
    ...
    try:
        ...
        db.commit()
        accumulated = len(data_msg.get("metrics", {}).get("data", []))
        return {
            "ok":              True,
            "value":           value,
            "baseline":        baseline,
...
def preview_message(metric_id: int, db: Session = Depends(get_db)):
    ...
    # Накопленные точки за 24ч для реалистичного превью
    historical_points = _collect_historical_points(m.metric_hash, metric_id, db)
