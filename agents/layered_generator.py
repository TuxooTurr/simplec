...
class LayeredGenerator:
    ...
    def generate_case_markdown(self, case_info, qa_doc):
        ...
        prompt = (
            ...
            "**Шаг 2:** Заполнить обязательные поля и отправить заявку на консультацию\n"
            ...
            "  Тело запроса: {\"type\": \"consultation\", \"description\": \"Тестовая заявка\"}\n"
            "  Ответ: 201 Created, тело {\"id\": \"REQ-001\", \"status\": \"created\"}\n"
            "- БД: таблица requests — новая запись, status = 'created', description = 'Тестовая заявка'\n\n"
            "ЯЗЫК ШАГА И ЯЗЫК ПРОВЕРОК — РАЗНЫЕ, ЭТО ГЛАВНОЕ ПРАВИЛО:\n"
            "- Шаг (**Шаг N:**) — ТОЛЬКО бизнес-язык: что делает человек в терминах предметной области.\n"
            "  Шаг должен быть понятен аналитику и заказчику, не только тестировщику.\n"
            "  В шаге ЗАПРЕЩЕНЫ: HTTP-методы (GET/POST/PUT/DELETE), URL и эндпоинты, curl, JSON/XML,\n"
            "  SQL, имена таблиц и полей, коды ответов, названия функций и параметров.\n"
            "  ПЛОХО: «Вызвать метод createIncident с параметром priority=3»\n"
            "  ХОРОШО: «Создать инцидент с приоритетом Высокий»\n"
            "  ПЛОХО: «Отправить POST /api/v1/transactions с body {amount: 1000}»\n"
            "  ХОРОШО: «Создать перевод на 1 000 ₽ со счёта клиента»\n"
            "  ПЛОХО: «Выполнить SELECT из таблицы requests»\n"
            "  ХОРОШО: «Убедиться, что заявка сохранилась в системе»\n"
            "- Зоны UI / API / БД — это блок «Результат» шага. Здесь наоборот нужна ТОЧНОСТЬ,\n"
            "  тестировщик сверяет по ним факт. Обтекаемые формулировки здесь — брак.\n"
            "  API — назови метод (эндпоинт или имя операции), перечисли параметры запроса\n"
            "  ПОИМЕННО со значениями, затем код ответа и параметры ответа поимённо со значениями.\n"
            "  Бизнес-значение из шага переведи в техническое: «приоритет Высокий» → priority = 3.\n"
            "  БД — схема и таблица, затем КАЖДЫЙ значимый столбец и что в него записалось.\n"
            "  Не «запись создана», а «таблица incidents: priority = 3, status = 'NEW', client_id = 42».\n"
            "  Если шаг задевает НЕСКОЛЬКО таблиц — назови каждую перед её столбцами. Иначе столбцы\n"
            "  второй таблицы читаются как столбцы первой, и проверить их нельзя.\n"
            "  Зону можно писать в несколько строк: строки-продолжения с отступом относятся к той же зоне.\n"
            "  ПРИМЕР связки «бизнес-шаг → точный результат»:\n"
            "  **Шаг N:** Создать инцидент с приоритетом Высокий\n"
            "  - Тестовые данные: тема «Не открывается касса», приоритет Высокий\n"
            "  - UI: Инцидент создан, в карточке приоритет «Высокий», статус «Новый»\n"
            "  - API: метод createIncident (POST /api/v1/incidents)\n"
            "    Параметры запроса: title = «Не открывается касса», priority = 3 (Высокий), clientId = 42\n"
            "    Ответ: 201 Created; параметры ответа: incidentId = «INC-001», status = «NEW», priority = 3\n"
            "  - БД: схема public\n"
            "    таблица incidents: столбец priority = 3, столбец status = 'NEW', столбец client_id = 42, "
            "столбец title = 'Не открывается касса', столбец created_at заполнен текущим временем\n"
            "    таблица escalations: столбец incident_id = 'INC-001', "
            "столбец notified_at заполнен текущим временем\n"
            "КРИТИЧЕСКИ ВАЖНО:\n"
            ...
            "- API: метод, параметры запроса поимённо, код и параметры ответа поимённо — "
            "если запроса нет, пиши \"Запросов к API нет\"\n"
            "- БД: схема, таблица, столбцы и записанные значения — если изменений нет, "
            "пиши \"Изменений в БД нет\"\n"
            "- В зоне БД КАЖДАЯ таблица идёт ОТДЕЛЬНОЙ строкой, начинающейся со слова «таблица». "
            "Строка без слова «таблица» в начале запрещена: непонятно, чьи это столбцы. "
            "Все столбцы одной таблицы — на её строке, через запятую.\n"
            "  ПЛОХО:  таблица incidents: столбец status = 'NEW';\n"
            "          столбец incident_id = 'INC-001', столбец notified_at ...\n"
            "  ХОРОШО: таблица incidents: столбец status = 'NEW'\n"
            "          таблица escalations: столбец incident_id = 'INC-001', столбец notified_at ...\n"
    ...
    def _parse_markdown(self, text, case_info):
        ...
        # Зоны API и БД описываются подробно (запрос, тело, ответ, таблицы), и
        # модель естественно разбивает их на несколько строк. Раньше бралась
        # только строка с заголовком зоны, а продолжение молча терялось — из
        # ожидаемого результата пропадали тело запроса и структура ответа.
        # Поэтому продолжения (строки без своего заголовка) приклеиваем к
        # текущей зоне через <br/>: так деталь доходит до кейса и в XML
        # остаётся читаемой построчно.
        ZONE_PATTERNS = [
            ("test_data", r'^[-*]?\s*(?:тестовые\s+данные|test\s*data)\s*:'),
            ("ui",        r'^[-*]?\s*ui\s*:'),
            ("api",       r'^[-*]?\s*api\s*:'),
            ("db",        r'^[-*]?\s*(?:бд|db|база\s+данных)\s*:'),
        ]
        ...
        for _num, block in matches:
            ...
            filled: set = set()      # зоны, куда уже что-то записали
            current: str | None = None   # зона, к которой относятся строки-продолжения
            ...
            for line in lines:
                ...
                key = next((k for k, pat in ZONE_PATTERNS if re.match(pat, ll)), None)
                if key:
                ...
                # Строка-продолжение: относится к последней открытой зоне.
                # Служебные строки вроде «- Результат:» зоны не открывают.
                if current and not ls.rstrip().endswith(":"):
                    cont = ls.lstrip("-*• \t").strip()
                    if not cont:
                        continue
                    if current in filled:
                        step[current] += "<br/>" + cont
                    else:
                        step[current] = cont
                        filled.add(current)
    ...
    def wrap_case_to_xml_via_llm(self, case, qa_doc,
        ...
        prompt = (
            ...
            '        <step index="0">\n'
            "            <customFields/>\n"
            "            <description><![CDATA[Действие на бизнес-языке, без методов и SQL]]></description>\n"
            "            <expectedResult><![CDATA[UI: что видит пользователь<br/>API: метод, endpoint, заголовки, тело запроса, код и структура ответа<br/>БД: таблица, поле, значение]]></expectedResult>\n"
            ...
            "3. expectedResult — это поле «Результат» шага: ВСЕГДА три зоны UI/API/БД через <br/>, "
            "если зона неприменима — пиши внутри неё «Не требуется», саму зону не пропускай. "
            "Здесь нужна ТОЧНОСТЬ, по этим зонам тестировщик сверяет факт:\n"
            "   API — метод (эндпоинт или имя операции), параметры запроса ПОИМЕННО со значениями, "
            "код ответа и параметры ответа ПОИМЕННО со значениями. Бизнес-значение из шага переводи "
            "в техническое: «приоритет Высокий» → priority = 3\n"
            "   БД — схема, таблица и КАЖДЫЙ значимый столбец с записанным значением. "
            "Не «запись создана», а «таблица incidents: priority = 3, status = 'NEW', client_id = 42». "
            "Если задето несколько таблиц — назови каждую перед её столбцами\n"
            "   Ничего из зон исходного Markdown-кейса не теряй — переноси все строки\n"
            "3.1. description — строго бизнес-язык: что делает человек в терминах предметной области. "
            "В шаге НЕ должно быть HTTP-методов, эндпоинтов, curl, JSON, SQL, имён таблиц, столбцов, "
            "функций, параметров и кодов ответов — всё это переносится в зоны API и БД. "
            "ПЛОХО: «Вызвать метод createIncident с параметром priority=3». "
            "ХОРОШО: «Создать инцидент с приоритетом Высокий». "
            "Бери формулировку шага из Markdown-кейса; если она техническая — переформулируй её "
            "на бизнес-язык, а технику убери в нужную зону\n"
